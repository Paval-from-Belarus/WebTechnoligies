...
/**
 * Configure common bean environment during application start up
 *
 * @param contextEvent the ServletContextEvent containing the ServletContext that is being initialized
 */
@Override
...
/**
 * Find classes with specific annotation and map them to {@code Map.entry<MyInterfaceName, Class<?>}<br>
 * If class have more than one interface, then self-class will be chosen as target
 * if several classes have same interface, then both will be chosen as target
 */
...
/**
 * Search beans annotated with specific annotation to put in the common bean map.
 * This method should be invoked first, because other stereotype beans ({@link Service}, {@link  RequestHandler} may depend on them).
 * Each dao bean also uses Mapper generated by {@link org.mapstruct} dependency with {@link Named} annotations
 *
 * @param pool              the pool to share connection to the database
 * @param daoBeansPackage   the package to scan where dao beans are storing
 * @param mapperBeanPackage the package to scan where mappers' beans are storing
 */
...
/**
 * Scan service beans and put them in the common bean map.
 * After scanning this method configure bean annotated with {@link RequestHandlerProvider}
 * by putting to corresponding beans with {@link RequestHandlerDefinition}.
 * The method
 * as {@link #initDaoBeans(ConnectionPool, String, String)} doesn't make any guarantee about the order
 * in which beans will be created.
 * That is impossible to inject service bean in another (possible, but may cause unpredictable behavior)
 *
 * @param serviceBeanPackage package where all (services, request handlers) bea
 */
...
/**
 * Puts beans in the instanceMap based on the given classSet map.
 *
 * @param classSet The map containing the target class and implementation class of the beans.
 */
@SneakyThrows
...
/**
 * Finds a bean in the instanceMap based on the given class.
 *
 * @param clazz The class of the bean to find.
